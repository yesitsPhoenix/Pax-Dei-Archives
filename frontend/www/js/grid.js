...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Add event listeners to reset the timeout for closing the sidebar
    sidebar.addEventListener("mouseover", resetSidebarTimeout);
    sidebar.addEventListener("mouseout", resetSidebarTimeout);
    mainMenus.forEach(menu => {
        menu.addEventListener("mouseover", resetSidebarTimeout);
    });

    // Start the initial timeout to close the sidebar after 4 seconds of inactivity
    closeSidebarAfterDelay();
    ...
    let submenuItems = document.querySelectorAll(".sub-menu-item, .sub-menu-map, .sub-menu-item-map");
...
document.addEventListener("DOMContentLoaded", function() {
    var mapsMenuItem = document.querySelector('#mapsSubMenu .sub-menu-item-map'); // Combined map
    var mapsSubMenuItems = document.querySelectorAll('#mapsSubMenu .sub-menu-map'); // individual maps
    ...
    if (mapsMenuItem && mapsSubMenuItems.length > 0 && mapsSubMenu && gridContainer && mapIframe) {
        ...
        document.addEventListener('click', function(event) {
            if (!mapsSubMenu.contains(event.target)) {//&& !mapsMenuItem.contains(event.target)) {
