...
export async function fetchSingleArticle(slug) {
    if (!slug) return null;
    try {
        const response = await fetch(`/articles/item/${slug}`);
        if (!response.ok) {
        ...
        if (result.status === 'success') {
            ...
            console.warn(`Article not found or API error: ${result.message}`);
            return null;
        }
    } catch (error) {
        console.error('Error in fetchSingleArticle:', error);
        return null;
    }
}
