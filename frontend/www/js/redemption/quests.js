        ...
        target.querySelectorAll('a').forEach(link => {
            ...
            try {
                ...
                if (questKey) {
                    ...
                    link.onclick = (e) => {
                        e.preventDefault();
                        const targetQuest = window.allQuests.find(q => q.quest_key === questKey);
                        if (targetQuest) {
                            const isClaimed = window.userClaims.some(c => c.quest_id === targetQuest.id);
                            showQuestDetails(targetQuest, isClaimed);
                            const detailsPanel = document.getElementById('details-content');
                            if (detailsPanel) detailsPanel.scrollTop = 0;
                        } else {
    ...
    if (prereqContainer) {
        if (prerequisites.length > 0) {
            const prereqList = prerequisites.map(id => {
                const q = window.allQuests.find(item => item.id === id);
